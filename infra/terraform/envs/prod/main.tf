...
locals {
  ...
  admin_cors_origins = var.enable_platform ? [
    "https://${local.admin_subdomain}.${var.root_domain}",
  ] : []
  portal_cors_origins = var.enable_platform ? [
...
module "dynamodb" {
  ...
  # Hyperscale config: Revert to PAY_PER_REQUEST as requested (feature flagged via var)
  rate_limits_billing_mode = "PAY_PER_REQUEST"
