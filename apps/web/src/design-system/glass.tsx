...
export const GlassCard = forwardRef<HTMLDivElement, GlassCardProps>(
  (
    { variant = 'light', padding = 'md', hover = true, glow = false, children, style, ...props },
    ref
  ) => {
        ...
        whileHover={
          !disabled && !loading
            ? {
                ...
                boxShadow:
                  variant === 'primary'
                    ? tokens.shadows.glowPurpleStrong
                    : tokens.shadows.glassHover,
              }
            : undefined
        }
...
export const GlassInput = forwardRef<HTMLInputElement, GlassInputProps>(
  ({ label, error, size = 'md', icon, style, className, ...props }, ref) => {
    ...
    const inputStyles: any = {
      ...
      boxShadow: focused
        ? tokens.shadows.glowPurple
        : error
          ? tokens.shadows.glowPurple.replace('139, 92, 246', '239, 68, 68')
          : 'none',
      transition: `all ${tokens.transitions.duration.fast} ${tokens.transitions.easing.default}`,
...
interface GlassTextareaProps extends Omit<
...
export const FAQAccordion: React.FC<FAQAccordionProps> = ({ items }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[3] }}>
      {items.map((item) => {
        // Generate a stable key from the question content
        ...
        return (
          ...
          >
            ...
            <motion.div
              initial={false}
              animate={{
                height: openIndex === itemIndex ? 'auto' : 0,
                opacity: openIndex === itemIndex ? 1 : 0,
                marginTop: openIndex === itemIndex ? tokens.spacing[4] : 0,
              }}
              transition={{ duration: 0.2 }}
        ...
        >
          {options.map((option) => (
            <option
              key={option.value}
              value={option.value}
              style={{ background: tokens.colors.background.tertiary }}
            >
              {option.label}
            </option>
          ))}
