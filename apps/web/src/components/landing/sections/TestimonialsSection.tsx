      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacing[8] }}>
        ...
        <motion.div variants={itemVariants}>
          ...
          >
            ...
            {/* Rating */}
            {featured.rating && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: tokens.spacing[1], marginBottom: tokens.spacing[6] }}>
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    width="28"
                    height="28"
                    viewBox="0 0 20 20"
                    fill={i < featured.rating! ? accentColor : tokens.colors.border.subtle}
                  >
            ...
            {/* Author */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: tokens.spacing[4] }}>
              {featured.avatar && (
                <img
                  src={featured.avatar}
                  alt={featured.author}
              ...
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontSize: tokens.typography.fontSize.lg, fontWeight: tokens.typography.fontWeight.semibold, color: tokens.colors.text.primary }}>
                  {featured.author}
                </div>
                {featured.role && (
                  <div style={{ fontSize: tokens.typography.fontSize.base, color: tokens.colors.text.muted }}>
                    {featured.role}
                  </div>
                )}
              </div>
