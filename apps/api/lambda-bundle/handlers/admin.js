...
import { getAllFlags, updateFeatureFlag } from '../lib/feature-flags.js';
...
import { MAX_BODY_SIZE, DB_PREFIXES, DB_SORT_KEYS, HTTP_HEADERS, HTTP_STATUS, ACTOR_TYPES, VALID_MEMBERSHIP_ROLES as VALID_ROLES_LIST, VALID_GRANULARITIES, ANALYTICS_GRANULARITY, } from '../lib/constants.js';
const log = createLogger('admin-handler');
// ---------------------------------------------------------------------------
// Validation constants
// ---------------------------------------------------------------------------
const VALID_MEMBERSHIP_ROLES = VALID_ROLES_LIST;
...
function mapPortalRoleToMembership(role) {
...
function mapMembershipRoleToAdmin(role) {
    switch (role) {
        case 'ORG_OWNER':
            return 'owner';
        case 'MANAGER':
...
function mapAdminRoleToMembership(role) {
    switch (role) {
        case 'owner':
            return 'ORG_OWNER';
        case 'admin':
        ...
        case 'member':
            return 'AGENT';
        default:
            return VALID_MEMBERSHIP_ROLES.includes(role) ? role : null;
    }
}
function mapPipelineToStatus(status) {
    switch (status) {
        case 'closed_won':
            return 'won';
        case 'closed_lost':
...
async function mapAdminRule(rule, orgCache, userCache) {
...
async function collectFunnelLeads(funnelId) {
    const leads = [];
    let cursor;
    let pages = 0;
    do {
...
function buildFunnelStats(funnelId, leads) {
    const byStatus = VALID_LEAD_STATUSES.reduce((acc, status) => ({ ...acc, [status]: 0 }), {});
    const byPipelineStatus = {
        none: 0,
        nurturing: 0,
        negotiating: 0,
        closing: 0,
...
async function collectRecentLeadsAcrossFunnels(funnelIds, limit = 10) {
    if (funnelIds.length === 0)
        return [];
    const perFunnel = Math.max(1, Math.ceil(limit / funnelIds.length));
    const results = await Promise.all(funnelIds.map((funnelId) => leadsDb.queryLeads({
        funnelId,
        limit: perFunnel,
    })));
...
async function handleAdminRequest(event) {
    ...
    try {
        ...
        if (subpath.startsWith('/gdpr/')) {
            return await handleGdprRoutes(event, subpath, method, canWrite, admin.emailHash, requestOrigin);
        }
        // ---- ORGS ----
        ...
        if (subpath === '/orgs' && method === 'GET') {
            const result = await orgsDb.listOrgs(queryParam(event, 'cursor'), Number(queryParam(event, 'limit')) || 25, queryParam(event, 'search') || undefined);
            return resp.success({
                orgs: result.items,
        ...
        if (subpath === '/users' && method === 'POST') {
            ...
            if (userType === 'portal') {
                ...
                if (existing)
                    return resp.conflict('User already exists', requestOrigin);
                const createOrg = body.createOrg === true;
                const orgIdInput = typeof body.orgId === 'string' ? body.orgId.trim() : '';
                const orgName = typeof body.orgName === 'string' ? body.orgName.trim() : '';
                const orgSlugInput = typeof body.orgSlug === 'string' ? body.orgSlug.trim().toLowerCase() : '';
                let createdOrg = null;
                ...
                const portalRole = createOrg ? 'admin' : portalRoleInput;
                const membershipRole = createOrg
                    ? 'ORG_OWNER'
        ...
        if (/^\/orgs\/[^/]+\/members$/.test(subpath) && method === 'POST') {
            ...
            const resolvedRole = mapAdminRoleToMembership(body.role);
            if (!resolvedRole) {
        ...
        if (/^\/orgs\/[^/]+\/members$/.test(subpath) && method === 'GET') {
            const orgId = pathParam(event, 2);
            const result = await membershipsDb.listOrgMembers(orgId, queryParam(event, 'cursor'), Number(queryParam(event, 'limit')) || 50);
            const memberUsers = await Promise.all(result.items.map(async (member) => ({
                member,
        ...
        if (/^\/orgs\/[^/]+\/members\/[^/]+$/.test(subpath) && method === 'PUT') {
            ...
            const resolvedRole = body.role
                ? (mapAdminRoleToMembership(body.role) ?? undefined)
                : undefined;
        ...
        if (subpath === '/rules' && method === 'GET') {
            ...
            const result = await rulesDb.listRules(funnelId || undefined, queryParam(event, 'cursor'), Number(queryParam(event, 'limit')) || 50);
        ...
        if (/^\/rules\/test$/.test(subpath) && method === 'POST') {
            ...
            if (!funnelId)
                return resp.badRequest('funnelId is required', requestOrigin);
            ...
            const adapted = rules.map((r) => ({
                ruleId: r.ruleId,
                funnelId: r.funnelId,
                targetType: r.targetUserId ? 'USER' : 'ORG',
        ...
        if (/^\/rules\/bulk-create$/.test(subpath) && method === 'POST') {
            ...
            for (let i = 0; i < ruleInputs.length; i++) {
                ...
                try {
                    const targetOrgId = (typeof input.targetOrgId === 'string' && input.targetOrgId.trim()) ||
        ...
        if (/^\/rules\/[^/]+$/.test(subpath) && method === 'GET') {
            ...
            if (!rule)
                return resp.notFound('Rule not found', requestOrigin);
        ...
        if (/^\/rules\/[^/]+$/.test(subpath) && method === 'PUT') {
            ...
            const funnelId = funnels.length === 1
                ? funnels[0]
            ...
            const zipPatterns = Array.isArray(body.zipCodes)
                ...
                : Array.isArray(body.zipPatterns)
                    ? body.zipPatterns.map((z) => String(z).trim()).filter(Boolean)
                    : undefined;
            const targetOrgId = typeof body.targetOrgId === 'string'
        ...
        if (subpath === '/query' && method === 'POST') {
            ...
            if (body === null)
                return resp.badRequest('Invalid JSON in request body', requestOrigin);
            const statusFromPipeline = mapPipelineToStatus(body.pipelineStatus);
            const funnelId = body.funnelId;
            const orgId = body.orgId;
            const status = body.status || statusFromPipeline;
            const hasFilter = Boolean(funnelId || orgId || status);
            if (!hasFilter) {
                return resp.success({ leads: [], totalCount: 0, nextToken: undefined }, undefined, requestOrigin);
            }
            const result = await leadsDb.queryLeads({
                funnelId,
                orgId,
                ...
                cursor: body.nextToken || body.cursor,
                limit: body.pageSize,
            ...
            if (body.search) {
                ...
                mapped = mapped.filter((lead) => lead.name.toLowerCase().includes(search) || lead.email.toLowerCase().includes(search));
            }
            if (body.zipCode) {
        ...
        if (subpath === '/leads/bulk-update' && method === 'POST') {
            ...
            for (const item of updates) {
                ...
                if (item.status && !VALID_LEAD_STATUSES.includes(item.status)) {
                    errors.push({
                        funnelId: item.funnelId,
                        leadId: item.leadId,
        ...
        if (subpath === '/leads/bulk-import' && method === 'POST') {
            ...
            if (leads) {
                ...
                const lines = csvData
                    ...
                    .filter(Boolean);
                if (lines.length < 2) {
                    return resp.badRequest('CSV must contain a header row and at least one data row', requestOrigin);
                }
            ...
            return resp.success({ imported, failed: failedCount, skipped: 0, errors: importErrors }, undefined, requestOrigin);
        }
        ...
        if (/^\/leads\/[^/]+\/[^/]+$/.test(subpath) && method === 'PUT') {
            ...
            if (desiredStatus && !VALID_LEAD_STATUSES.includes(desiredStatus)) {
                return resp.badRequest(`Invalid status. Must be one of: ${VALID_LEAD_STATUSES.join(', ')}`, requestOrigin);
            }
            ...
            const notes = typeof body.notes === 'string' ? [body.notes] : body.notes;
        ...
        if (/^\/leads\/[^/]+\/[^/]+\/reassign$/.test(subpath) && method === 'POST') {
            ...
            if (body === null)
                return resp.badRequest('Invalid JSON in request body', requestOrigin);
            if (!body.orgId)
                return resp.badRequest('orgId is required', requestOrigin);
            const updated = await leadsDb.reassignLead(funnelId, leadId, body.orgId, body.ruleId);
            await recordAudit({
                actorId: admin.emailHash,
                actorType: ACTOR_TYPES.ADMIN,
        ...
        if (subpath === '/leads/reassign' && method === 'POST') {
            ...
            const updated = await leadsDb.reassignLead(body.funnelId, body.leadId, body.targetOrgId, body.ruleId);
        ...
        if (subpath === '/dashboard' && method === 'GET') {
            const funnelIds = (process.env.FUNNEL_IDS || '')
                ...
                .map((id) => id.trim())
                .filter(Boolean);
            const funnelStats = await Promise.all(funnelIds.map(async (funnelId) => {
            ...
            const unassignedLeads = funnelStats.reduce((sum, stat) => sum + (stat.byStatus.unassigned || 0), 0);
        ...
        if (subpath === '/notifications' && method === 'GET') {
            ...
            const result = await notificationsDb.listNotifications(cursor, limit, queryParam(event, 'startDate'), queryParam(event, 'endDate'));
            ...
            return resp.success({ notifications, total: notifications.length, nextToken: result.nextCursor }, undefined, requestOrigin);
        }
        ...
        if (subpath === '/exports' && method === 'POST') {
            ...
            const myPending = pendingExports.items.filter((e) => e.requestedBy === admin.emailHash && e.status === 'pending');
            if (myPending.length >= 3) {
                return resp.error('RATE_LIMITED', 'Too many pending exports. Please wait for current exports to complete.', HTTP_STATUS.RATE_LIMITED, undefined, requestOrigin);
            }
            ...
            try {
                ...
                if (err &&
                    ...
                    return resp.error('RATE_LIMITED', 'Please wait before creating another export', HTTP_STATUS.RATE_LIMITED, undefined, requestOrigin);
                }
                throw err;
            }
        ...
        if (subpath === '/exports' && method === 'GET') {
            ...
            return resp.success({ exports: exportsList, total: exportsList.length, nextToken: result.nextCursor }, undefined, requestOrigin);
        }
        ...
        if (subpath === '/exports/status' && method === 'GET') {
            ...
            if (!jobId)
                return resp.badRequest('jobId is required', requestOrigin);
...
function mapMembershipRoleToAdminRole(role) {
    switch (role) {
        case 'ORG_OWNER':
            return 'owner';
        case 'MANAGER':
...
async function mapAdminUser(user) {
...
async function mapAdminUserDetail(user) {
    const memberships = await membershipsDb.listUserOrgs(user.userId, undefined, 50);
    const orgs = await Promise.all(memberships.items.map(async (membership) => {
...
// ---------------------------------------------------------------------------
async function handleAnalyticsRoutes(event, subpath, method, requestOrigin) {
    if (method !== 'GET')
        return resp.notFound('Analytics endpoint not found', requestOrigin);
    const startDate = queryParam(event, 'startDate') || new Date(Date.now() - 30 * 86400000).toISOString();
    ...
    if (subpath === '/analytics/trends') {
        ...
        if (!VALID_GRANULARITIES.includes(granularity)) {
            return resp.badRequest(`granularity must be one of: ${VALID_GRANULARITIES.join(', ')}`, requestOrigin);
        }
...
// GDPR Routes (Task 7)
// ---------------------------------------------------------------------------
async function handleGdprRoutes(event, subpath, method, canWrite, adminEmailHash, requestOrigin) {
    const doc = getDocClient();
    // POST /admin/gdpr/erasure
    if (subpath === '/gdpr/erasure' && method === 'POST') {
        ...
        if (!emailHash)
            return resp.badRequest('emailHash is required', requestOrigin);
        // Find all leads matching the email hash
        const results = await doc.send(new ScanCommand({
        ...
        for (const lead of leads) {
            try {
              ...
              #msg = :redacted, #ip = :redacted, userAgent = :redacted`,
                ...
                const msg = err instanceof Error ? err.message : 'Unknown error';
                log.error('gdpr.erasure.leadFailed', { pk: lead.pk, error: msg });
            }
        }
