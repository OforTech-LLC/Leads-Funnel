...
import { generateNormalizedLead, DISPOSABLE_EMAIL_DOMAINS } from './helpers.js';
// Mock the hash module since we want to test security logic, not hashing
vi.mock('../lib/hash.js', () => ({
    hashIp: vi.fn((ip, salt) => `hashed_${ip}_${salt}`),
    hashEmailWithSalt: vi.fn((email, _salt) => `hashed_${email}`),
    generateIdempotencyKey: vi.fn((email, pageUrl, bucket) => `idem_${email}_${pageUrl || 'nourl'}_${bucket}`),
}));
...
describe('analyzeLeadSecurity', () => {
    ...
    describe('spam keyword detection', () => {
        ...
        it('should not flag legitimate messages', () => {
            const lead = generateNormalizedLead({
                email: 'test@example.com',
                message: 'I am interested in your roofing services. Please call me to schedule an estimate.',
            });
            const result = analyzeLeadSecurity(lead, defaultIp, defaultSalt, defaultWindowMinutes);
            expect(result.suspicious).toBe(false);
    ...
    describe('combined security analysis', () => {
        ...
        it('should flag lead with all suspicious indicators', () => {
            const lead = generateNormalizedLead({
                ...
                message: 'Get crypto gains at https://scam1.com and https://scam2.com for guaranteed income!',
            });
            const result = analyzeLeadSecurity(lead, defaultIp, defaultSalt, defaultWindowMinutes);
            expect(result.suspicious).toBe(true);
