 ...
 */
async function maybeDispatchWebhook(eventType, data) {
    try {
 ...
 */
export async function publishLeadCreatedEvent(config, lead, security) {
    ...
    void maybeDispatchWebhook(EVENT_TYPES.LEAD_CREATED, {
        leadId: lead.leadId,
        status: lead.status,
        createdAt: lead.createdAt,
        suspicious: security.suspicious,
 ...
 */
export async function emitLeadAssigned(region, eventBusName, detail) {
    ...
    // Dispatch webhook (fire-and-forget)
    void maybeDispatchWebhook(EVENT_TYPES.LEAD_ASSIGNED, {
 ...
 */
export async function emitLeadUnassigned(region, eventBusName, leadId, funnelId, zipCode, reason) {
    const client = getEventBridgeClient(region);
    const detail = {
        leadId,
        funnelId,
...
export async function emitLeadStatusChanged(data) {
    void maybeDispatchWebhook(EVENT_TYPES.LEAD_STATUS_CHANGED, data);
}
/**
...
export async function emitLeadNoteAdded(data) {
    void maybeDispatchWebhook(EVENT_TYPES.LEAD_NOTE_ADDED, data);
}
