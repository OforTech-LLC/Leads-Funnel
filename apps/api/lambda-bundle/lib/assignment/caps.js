 ...
 */
function getDailyTtl() {
    const now = new Date();
    const tomorrow = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 2, // day after tomorrow for safety buffer
    0, 0, 0));
 ...
 */
async function atomicCapCheck(tableName, region, pk, cap, ttl) {
 ...
 * @param pk - Partition key for the counter record
 */
async function decrementCap(tableName, region, pk) {
    const client = getDocClient(region);
    const sk = DB_SORT_KEYS.COUNTER;
    try {
        await client.send(new UpdateCommand({
 ...
 */
export async function checkAndIncrementCap(tableName, region, ruleId, dailyCap, monthlyCap) {
    ...
    if (dailyCap !== undefined && dailyCap > 0) {
        ...
        if (monthlyCap !== undefined && monthlyCap > 0) {
            ...
            const monthlyAllowed = await atomicCapCheck(tableName, region, monthlyPk, monthlyCap, getMonthlyTtl());
            if (!monthlyAllowed) {
