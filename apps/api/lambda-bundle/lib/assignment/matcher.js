 ...
 */
export function matchZipPattern(zipCode, patterns) {
 ...
 */
function matchGeoRadius(zipCode, rule) {
 ...
 */
async function roundRobinSelect(funnelId, priority, tiedRules) {
    ...
    try {
        ...
        const lastIndex = result.Attributes?.lastIndex ?? 0;
 ...
 */
export function matchLeadToRule(funnelId, zipCode, rules) {
 ...
 */
export async function matchLeadToRuleAsync(funnelId, zipCode, rules) {
    if (!rules || rules.length === 0) {
        return null;
    }
    // Step 1: Filter to active rules matching this funnelId (or wildcard)
