 ...
 */
export function anonymizeIPv4(ip) {
 ...
 */
export function anonymizeIPv6(ip) {
 ...
 */
export function analyzeLeadSecurity(lead, clientIp, ipHashSalt, windowMinutes) {
    const reasons = [];
    ...
    const idempotencyKey = generateIdempotencyKey(lead.email, lead.pageUrl ?? 'unknown', windowBucket);
    return {
 ...
 */
export function prepareAuditLogData(leadId, clientIp, email, ipHashSalt) {
