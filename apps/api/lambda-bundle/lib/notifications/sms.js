...
let cachedTwilioCredentials = null;
 ...
 */
async function loadTwilioCredentials(region, secretArn) {
    const now = Date.now();
 ...
 */
async function sendViaTwilio(to, body, region, secretArn) {
    try {
 ...
 */
async function sendViaSns(to, body, region) {
    try {
 ...
 */
export function buildLeadAssignedSms(lead) {
 ...
 * @returns SMS body text
 */
export function buildLeadUnassignedSms(lead, reason) {
    const funnelId = lead.funnelId || 'unknown';
