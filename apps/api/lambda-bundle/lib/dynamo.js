 ...
 */
export async function checkRateLimit(config, ipHash) {
 ...
 */
export async function checkIdempotency(config, idempotencyKey, leadId, status) {
    const client = getDocClient(config.awsRegion);
 ...
 */
export async function storeLead(config, tableName, lead, security, userAgent, score, options) {
    const client = getDocClient(config.awsRegion);
 ...
 */
export async function updateLeadAnalysis(config, tableName, leadId, analysis) {
    const client = getDocClient(config.awsRegion);
 ...
 * Get a lead by ID
 */
export async function getLead(config, tableName, leadId) {
    const client = getDocClient(config.awsRegion);
    const pk = `${DB_PREFIXES.LEAD}${leadId}`;
    const sk = DB_SORT_KEYS.META;
