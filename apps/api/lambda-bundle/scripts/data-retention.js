 ...
 */
function getArchiveEmailHashSalt() {
 ...
 */
async function archiveLeads(config, funnelId, leads) {
 ...
 */
async function deleteLeadsBatch(tableName, leads, dryRun) {
    ...
    for (let i = 0; i < leads.length; i += 25) {
        ...
        try {
            ...
            while (unprocessedItems && unprocessedItems.length > 0 && retryCount < 3) {
                ...
                const retryResult = await ddb.send(new BatchWriteCommand({
 ...
 */
async function processRetentionForFunnel(config, funnelId, cutoffDate) {
    const ddb = getDocClient();
 ...
 */
export async function executeRetention(config) {
