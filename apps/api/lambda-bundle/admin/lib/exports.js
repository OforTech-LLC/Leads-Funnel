 ...
 */
export async function createExportJob(config, user, request) {
    ...
    try {
        ...
        await updateJobStatus(config, job, 'failed', error instanceof Error ? error.message : 'Unknown error');
        throw error;
    }
    // Return updated job
    return getExportJob(config, user.sub, jobId);
}
/**
 * Get export job by ID
 */
export async function getExportJob(config, userId, jobId) {
    const ddb = getDocClient();
 ...
 * Update job status
 */
async function updateJobStatus(config, job, status, errorMessage, s3Key, recordCount) {
    const ddb = getDocClient();
 ...
 */
async function processExportJob(config, job, request) {
 ...
 */
async function generateExportContent(leads, format, fields) {
    const defaultFields = [
        'leadId',
        'email',
        'phone',
 ...
 */
async function generateXlsx(leads, fields) {
 ...
 */
async function generatePdf(leads, fields) {
    ...
    for (const lead of leads) {
        const values = fields.map((f) => String(lead[f] || ''));
 ...
 */
async function generateDocx(leads, fields) {
 ...
 */
export async function getDownloadUrl(config, job) {
    if (!job.s3Key) {
