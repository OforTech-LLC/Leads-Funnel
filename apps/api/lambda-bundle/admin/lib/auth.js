 ...
 */
async function getParameter(path) {
 ...
 */
export async function isFeatureEnabled(config) {
 ...
 */
async function getAllowedEmails(config) {
 ...
 */
async function getIpAllowlist(config) {
 ...
 */
async function verifyToken(authHeader, config) {
    if (!authHeader) {
...
function determineRole(groups) {
    ...
    if (groups.includes('Viewer') || groups.includes('OrgViewer'))
        return 'Viewer';
 ...
 */
export function extractClientIp(headers, sourceIp) {
 ...
 */
export async function authenticateAdmin(authHeader, clientIp, config) {
    ...
    const adminGroups = Array.from(new Set(groups.flatMap((group) => {
        if (group === 'Admin' || group === 'SuperAdmin' || group === 'OrgAdmin')
            return ['Admin'];
        if (group === 'Viewer' || group === 'OrgViewer')
            return ['Viewer'];
        return [];
    })));
