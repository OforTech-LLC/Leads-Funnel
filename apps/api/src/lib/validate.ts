 ...
 */
export function validateCustomFields(customFields: unknown): {
  valid: boolean;
  ...
  for (const key of keys) {
    ...
    if (!CUSTOM_FIELD_KEY_REGEX.test(key)) {
      errors.push(
        `customFields key "${key}" must start with a letter and contain only alphanumeric characters, underscores, and hyphens`
      );
      continue;
    }
    ...
    if (key.length > MAX_LENGTHS.customFieldKey) {
      errors.push(
        `customFields key "${key}" must be ${MAX_LENGTHS.customFieldKey} characters or less`
      );
      continue;
    }
    ...
    if (value.length > MAX_LENGTHS.customFieldValue) {
      errors.push(
        `customFields value for "${key}" must be ${MAX_LENGTHS.customFieldValue} characters or less`
      );
      continue;
    }
