...
const FLAG_ALIASES: Record<FeatureFlagName, readonly string[]> = {
...
export function normalizeFeatureFlags(
  ...
  const normalized: Record<FeatureFlagName, boolean> = { ...FLAG_DEFAULTS };

  (Object.keys(FLAG_DEFAULTS) as FeatureFlagName[]).forEach((flag) => {
...
export function pickFeatureFlags<T extends readonly FeatureFlagName[]>(
