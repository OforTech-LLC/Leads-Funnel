...
type BedrockRuntimeModule = {
  BedrockRuntimeClient: new (args: { region: string; requestHandler: NodeHttpHandler }) => {
    send: (
      command: unknown,
...
type BedrockRuntimeClientInstance = InstanceType<BedrockRuntimeModule['BedrockRuntimeClient']>;
...
const URGENCY_VALUES: LeadAnalysis['urgency'][] = ['high', 'medium', 'low'];
const INTENT_VALUES: LeadAnalysis['intent'][] = [
  'info_gathering',
  'ready_to_buy',
  'complaint',
  'other',
...
type BedrockProvider = 'anthropic' | 'titan' | 'unknown';
...
function normalizeAnalysis(raw: unknown): LeadAnalysis | null {
  ...
  const urgency = URGENCY_VALUES.includes(urgencyRaw as LeadAnalysis['urgency'])
    ? (urgencyRaw as LeadAnalysis['urgency'])
    : 'low';
  const intent = INTENT_VALUES.includes(intentRaw as LeadAnalysis['intent'])
    ? (intentRaw as LeadAnalysis['intent'])
    : 'other';
...
function heuristicAnalysis(text: string): LeadAnalysis {
...
let bedrockClient: BedrockRuntimeClientInstance | null = null;
let bedrockRuntimeModule: BedrockRuntimeModule | null = null;

function loadBedrockRuntime(): BedrockRuntimeModule | null {
  if (bedrockRuntimeModule) return bedrockRuntimeModule;
  try {
    bedrockRuntimeModule = require('@aws-sdk/client-bedrock-runtime') as BedrockRuntimeModule;
...
function getBedrockClient(region: string, timeoutMs: number) {
