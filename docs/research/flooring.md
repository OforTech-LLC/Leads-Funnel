# Flooring Company Landing Page Patterns

## Reference Landing Pages

1. **Empire Today** - https://www.empiretoday.com/
2. **Floor & Decor** - https://www.flooranddecor.com/
3. **LL Flooring** - https://www.llflooring.com/
4. **50 Floor** - https://www.50floor.com/
5. **Luna Flooring** - https://www.lunaflooring.com/
6. **National Floors Direct** - https://www.nationalfloorsdirect.com/
7. **The Home Depot Flooring** - https://www.homedepot.com/b/Flooring/N-5yc1vZaq7r
8. **Lowe's Flooring** - https://www.lowes.com/c/Flooring
9. **Carpet One** - https://www.carpetone.com/
10. **Flooring America** - https://www.flooringamerica.com/

---

## Above-Fold Patterns

### Headline Formulas
- **Transformation-focused**: "Transform Your Home with Beautiful Floors"
- **Speed-focused**: "New Floors Installed as Soon as Tomorrow"
- **Price-focused**: "Quality Flooring at Unbeatable Prices"
- **Selection-focused**: "Thousands of Styles. One Perfect Floor."
- **Service-focused**: "Shop-at-Home Flooring Services"
- **Guarantee-focused**: "Love Your Floors or We'll Replace Them"

### Hero Section Elements
1. **Room visualizer tool** prominently featured
2. **Flooring type navigation** (hardwood, carpet, vinyl, tile)
3. **Free in-home estimate CTA**
4. **Price guarantee badge**
5. **"Shop at Home" service highlight**
6. **Beautiful room imagery** with various floor types

### Visual Patterns
- Large, inspiring room photography
- Flooring close-up textures
- Before/after room transformations
- Interactive visualizer tools
- Clean, modern design aesthetic

---

## CTA Patterns

### Primary CTA Text
- "Get a Free Estimate"
- "Schedule Free In-Home Appointment"
- "Shop at Home - It's Free"
- "See It in Your Room"
- "Get Your Free Quote"
- "Find Your Perfect Floor"

### CTA Design Patterns
- **"Shop at Home" positioning** as primary differentiator
- **Room visualizer CTA** for engagement
- **Phone number prominently displayed**
- **"Next Day Installation" messaging**
- **Price match guarantees** near CTA

### Secondary CTAs
- "Browse Products"
- "See Flooring Styles"
- "View Special Offers"
- "Find a Store"

---

## Form Field Patterns

### Free Estimate Form (5-6 fields)

**Essential Fields**
- Name
- Phone number
- Email address
- ZIP code
- Room(s) to be floored (dropdown or checkbox)
- Preferred flooring type (optional)

### Shop-at-Home Appointment Form

**Step 1: Service Selection**
- What rooms need flooring?
- What type of flooring interests you? (hardwood, carpet, vinyl, tile, unsure)

**Step 2: Home Details**
- Approximate square footage
- Timeline (ASAP, 1-3 months, 3-6 months, just browsing)

**Step 3: Appointment Scheduling**
- Preferred date (calendar)
- Preferred time slot (morning, afternoon, evening)

**Step 4: Contact Info**
- Name, phone, email
- Address (for appointment)

### Form Best Practices
- **Room visualizer integration** before form
- **Flooring type pre-selection** shows interest
- **"Free, no-obligation" messaging** prominent
- **Evening/weekend appointment** options
- **Instant confirmation** with appointment details

---

## Trust Signals

### Price & Quality Guarantees
- "Lowest Price Guarantee"
- "We'll Beat Any Competitor's Price"
- "Love Your Floor Guarantee"
- "Lifetime Installation Warranty"
- Free installation promotions

### Industry Credentials
- Better Business Bureau rating
- Licensed and insured installers
- Manufacturer certifications
- Years in business
- Number of installations completed

### Social Proof Elements
- Customer review ratings
- "X Million Homes Floored"
- Review platform integrations
- Before/after galleries
- Video testimonials

### Quality Indicators
- Brand partnerships (Shaw, Mohawk, Armstrong)
- Product warranties
- Professional installation guarantee
- Environmentally friendly options

---

## Section Structure

### Recommended Page Flow

1. **Hero Section**
   - Compelling headline + room imagery
   - Shop-at-home CTA or visualizer
   - Trust indicators (price guarantee, years, rating)
   - Phone number

2. **Flooring Type Navigation**
   - Hardwood
   - Laminate
   - Vinyl/LVP
   - Carpet
   - Tile
   - Each with image and "Explore" link

3. **Room Visualizer Tool**
   - Upload room photo or select preset
   - Try different flooring options
   - Generate lead from visualization

4. **How It Works**
   - Step 1: Free In-Home Consultation
   - Step 2: Shop Samples in Your Home
   - Step 3: Professional Installation
   - Step 4: Love Your New Floors

5. **Why Choose Us**
   - Free shop-at-home service
   - Lowest price guarantee
   - Professional installation
   - Wide selection

6. **Special Offers**
   - Current promotions
   - Financing options
   - Bundle deals

7. **Portfolio/Gallery**
   - Completed installation photos
   - Room type filtering
   - Flooring type filtering

8. **Testimonials**
   - Customer quotes
   - Before/after photos
   - Star ratings

9. **Service Areas**
   - ZIP code checker
   - Map of service area

10. **FAQ Section**
    - Product questions
    - Installation questions
    - Pricing questions

11. **Final CTA**
    - Appointment scheduling form
    - Phone number
    - Chat option

---

## Objection Handling

### Common Objections & Responses

| Objection | Landing Page Response |
|-----------|----------------------|
| "Flooring is too expensive" | Price match guarantee, financing options, "starting at" pricing |
| "I don't know what type I need" | Free consultation, room visualizer, style guides |
| "Installation will be disruptive" | Fast installation timeline, professional installers, furniture moving |
| "I'm worried about quality" | Brand partnerships, product warranties, satisfaction guarantee |
| "I want to see it first" | Shop-at-home service, bring samples to you |
| "Hidden costs concern me" | All-inclusive pricing, no hidden fees messaging |
| "I need to measure first" | Free in-home measurement included |

### FAQ Questions to Include
1. What type of flooring is best for my room?
2. How long does installation take?
3. Do you move furniture?
4. What's included in the price?
5. Do you offer financing?
6. How do I maintain my new floors?
7. What's your warranty?
8. Can I see samples before deciding?

---

## Conversion Blueprint

### High-Converting Flooring Landing Page Formula

```
┌─────────────────────────────────────────────────────────────┐
│  HERO SECTION                                                │
│  ├─ Headline: "Beautiful New Floors, Installed Tomorrow"    │
│  ├─ Subheadline: Free in-home shopping experience           │
│  ├─ Hero image: Stunning room with beautiful flooring       │
│  ├─ CTA: "Schedule Your Free In-Home Appointment"           │
│  └─ Trust strip: Lowest Price Guarantee | X Million Homes   │
├─────────────────────────────────────────────────────────────┤
│  FLOORING TYPES (5-6 cards)                                 │
│  ├─ Hardwood [image] - "Timeless elegance"                  │
│  ├─ Vinyl/LVP [image] - "Waterproof & durable"              │
│  ├─ Laminate [image] - "Affordable style"                   │
│  ├─ Carpet [image] - "Comfort underfoot"                    │
│  ├─ Tile [image] - "Versatile beauty"                       │
│  └─ Each with "Shop Now" link                               │
├─────────────────────────────────────────────────────────────┤
│  ROOM VISUALIZER (Interactive)                              │
│  ├─ "See Your New Floors Before You Buy"                    │
│  ├─ Upload your photo or select room                        │
│  ├─ Try different flooring styles                           │
│  └─ "Get a Quote for This Look" CTA                         │
├─────────────────────────────────────────────────────────────┤
│  HOW IT WORKS (4 steps)                                     │
│  ├─ Step 1: Schedule Free Appointment                       │
│  ├─ Step 2: We Bring Samples to You                         │
│  ├─ Step 3: Choose Your Perfect Floor                       │
│  └─ Step 4: Professional Installation                       │
├─────────────────────────────────────────────────────────────┤
│  WHY CHOOSE US                                              │
│  ├─ Free Shop-at-Home Service                               │
│  ├─ Lowest Price Guarantee                                  │
│  ├─ Next-Day Installation Available                         │
│  └─ Lifetime Installation Warranty                          │
├─────────────────────────────────────────────────────────────┤
│  SPECIAL OFFERS                                             │
│  ├─ Current promotion banner                                │
│  ├─ Financing options (0% APR for 12 months)                │
│  └─ "Free Installation" or similar offer                    │
├─────────────────────────────────────────────────────────────┤
│  CUSTOMER GALLERY                                           │
│  ├─ Before/after photos                                     │
│  ├─ Room type categories                                    │
│  ├─ Flooring type labels                                    │
│  └─ "See More Projects" link                                │
├─────────────────────────────────────────────────────────────┤
│  TESTIMONIALS                                               │
│  ├─ Customer reviews (3-4)                                  │
│  ├─ Star ratings                                            │
│  ├─ Project photos                                          │
│  └─ Google/Yelp review widget                               │
├─────────────────────────────────────────────────────────────┤
│  FAQ SECTION                                                │
│  ├─ Product selection questions                             │
│  ├─ Installation questions                                  │
│  ├─ Pricing/warranty questions                              │
│  └─ Maintenance questions                                   │
├─────────────────────────────────────────────────────────────┤
│  FINAL CTA SECTION                                          │
│  ├─ "Ready for New Floors?"                                 │
│  ├─ Appointment scheduling form                             │
│  ├─ Phone number                                            │
│  └─ "Or Call Now: (XXX) XXX-XXXX"                           │
└─────────────────────────────────────────────────────────────┘
```

### Key Metrics to Target
- **Conversion rate**: 8-12% for appointment requests
- **Visualizer engagement**: Track tool usage
- **Appointment show rate**: Monitor no-shows
- **Average order value**: Track upsells

### A/B Testing Priorities
1. Hero imagery (room shot vs. flooring close-up)
2. CTA text ("Free Quote" vs. "Shop at Home" vs. "See Prices")
3. Visualizer placement (hero vs. dedicated section)
4. Form length (quick vs. detailed)
5. Price display (visible vs. hidden until appointment)

---

## Industry-Specific Insights

### What Makes Flooring Different
- **Visual decision** - Room visualizers highly effective
- **Shop-at-home model** - Key differentiator for many companies
- **Installation bundled** - Total price matters
- **Room-specific needs** - Kitchen vs. bedroom vs. bathroom

### Product-Specific Landing Pages
Create separate pages for:
- Hardwood flooring
- Luxury vinyl plank (LVP)
- Carpet
- Tile
- Laminate

Each with tailored benefits and imagery

### Room Visualizer Best Practices
- Empire Today's visualizer increases engagement significantly
- Allow photo upload AND preset rooms
- Show multiple flooring options
- Lead capture after visualization

### Financing Messaging
- 0% APR financing options
- "As low as $X/month" pricing
- Buy now, pay later options
- Credit approval messaging

### SEO Focus Areas
- "Flooring installation [city]"
- "Hardwood floors near me"
- "[Flooring type] installation cost"
- "Best flooring for [room type]"
