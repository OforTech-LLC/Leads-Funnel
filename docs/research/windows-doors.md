# Windows & Doors Landing Page Patterns

## Reference Landing Pages

1. **Renewal by Andersen** - https://www.renewalbyandersen.com/
2. **Pella Windows & Doors** - https://www.pella.com/
3. **Marvin Windows** - https://www.marvin.com/
4. **Andersen Windows** - https://www.andersenwindows.com/
5. **Champion Windows** - https://www.championwindow.com/
6. **Window World** - https://www.windowworld.com/
7. **Milgard Windows** - https://www.milgard.com/
8. **Simonton Windows** - https://www.simonton.com/
9. **Feldco Windows** - https://www.feldco.com/
10. **Window Nation** - https://www.windownation.com/

---

## Above-Fold Patterns

### Headline Formulas
- **Savings-focused**: "Cut Energy Bills with New Windows"
- **Quality-focused**: "Premium Windows & Doors, Expertly Installed"
- **Problem-solving**: "Drafty Windows? We Can Help."
- **Trust-focused**: "America's #1 Window Replacement Company"
- **Offer-focused**: "Get 40% Off Windows + Free Installation"
- **Transformation**: "Beautiful Windows That Transform Your Home"

### Hero Section Elements
1. **Current promotion** prominently displayed
2. **Free consultation CTA** above fold
3. **Energy savings calculator** or preview
4. **Product type navigation** (windows, doors, patio doors)
5. **Trust indicators** (years in business, warranties)
6. **Phone number** highly visible
7. **Before/after imagery** or beautiful window shots

### Visual Patterns
- Beautiful home exteriors with new windows
- Interior shots showing natural light
- Window/door product close-ups
- Before/after comparisons
- Clean, professional design aesthetic

---

## CTA Patterns

### Primary CTA Text
- "Get a Free Quote"
- "Schedule Your Free Consultation"
- "Request Free Estimate"
- "Book Your Appointment"
- "Get $X Off - Request Quote"
- "See Your Savings"

### CTA Design Patterns
- **Promotion-driven** (discount offers prominent)
- **Phone number as co-equal CTA**
- **"Free, no-obligation" messaging**
- **Urgency elements** ("Limited time offer")
- **Dual CTAs** (online form + phone)

### Secondary CTAs
- "View Products"
- "See Windows"
- "Explore Styles"
- "Calculate Savings"

---

## Form Field Patterns

### Free Quote Form (5-6 fields)

**Essential Fields**
- Name
- Phone number
- Email address
- ZIP code
- Project type (Windows, Entry Doors, Patio Doors, All)
- Number of windows/doors (dropdown range)

### Detailed Consultation Form

**Step 1: Project Scope**
- What are you replacing? (Windows, Entry Door, Patio Door, Sliding Door)
- Number of windows to replace
- Number of doors to replace

**Step 2: Home Details**
- Home type (Single family, Townhome, Condo)
- Home age (for energy savings calculation)
- Current window type/age

**Step 3: Preferences**
- Timeline (ASAP, 1-3 months, 3-6 months, just getting info)
- Style preferences (if known)
- Energy efficiency priority

**Step 4: Contact & Scheduling**
- Name, phone, email
- Address
- Preferred appointment time

### Form Best Practices
- **Project scope first** (windows, doors, both)
- **Number of units** helps qualification
- **Energy savings messaging** throughout
- **"See Your Price" as submit button**
- **No-pressure, free consultation** emphasis

---

## Trust Signals

### Industry Credentials
- Licensed and Insured badge
- ENERGY STAR Partner
- AAMA (American Architectural Manufacturers Association)
- Lead-Safe Certified
- Better Business Bureau rating

### Product Certifications
- ENERGY STAR certified products
- NFRC (National Fenestration Rating Council) ratings
- Product performance ratings displayed
- Material certifications (Fibrex, Ultrex, etc.)

### Social Proof Elements
- Years in business ("Since 1903")
- Number of installations ("1 Million+ Windows Installed")
- Customer review ratings
- Video testimonials
- "As seen on This Old House"
- Consumer Reports ratings

### Warranties & Guarantees
- Product warranties (20-year, lifetime)
- Installation warranty
- Transferable warranty messaging
- Satisfaction guarantee

---

## Section Structure

### Recommended Page Flow

1. **Hero Section**
   - Current promotion headline
   - Hero image of beautiful windows/home
   - Free consultation CTA
   - Trust strip (years, installations, warranty)
   - Phone number

2. **Product Categories**
   - Replacement Windows
   - Entry Doors
   - Patio Doors
   - Storm Windows/Doors
   - Each with image and "Explore" link

3. **Energy Savings Calculator**
   - Interactive tool
   - Input: Window count, home age, location
   - Output: Estimated annual savings
   - Lead capture integration

4. **Why Replace Windows**
   - Energy efficiency benefits
   - Comfort improvements
   - Curb appeal enhancement
   - Noise reduction
   - Security upgrades

5. **Our Process**
   - Free In-Home Consultation
   - Custom Measurements
   - Professional Installation
   - Final Inspection
   - Warranty Registration

6. **Product Showcase**
   - Window styles (double-hung, casement, bay, etc.)
   - Door styles (entry, French, sliding)
   - Material options
   - Color/finish options

7. **Why Choose Us**
   - Manufacturer-direct (where applicable)
   - Expert installation
   - Industry-leading warranty
   - Financing options

8. **Testimonials**
   - Customer quotes
   - Before/after photos
   - Video testimonials
   - Star ratings

9. **Financing Section**
   - Monthly payment calculator
   - 0% APR offers
   - Easy qualification messaging

10. **FAQ Section**
    - Cost questions
    - Timeline questions
    - Product questions
    - Warranty questions

11. **Final CTA**
    - Consultation form
    - Phone number
    - Current promotion reminder

---

## Objection Handling

### Common Objections & Responses

| Objection | Landing Page Response |
|-----------|----------------------|
| "New windows are too expensive" | Energy savings calculator, financing options, ROI messaging |
| "I can wait" | Urgency (energy costs rising, limited-time offers), comfort benefits |
| "I'm worried about installation quality" | Certified installers, warranty, reviews, process transparency |
| "How do I know which windows to choose?" | Free expert consultation, style guide, product comparison |
| "It will be disruptive" | Installation timeline (1-day for many projects), professional process |
| "I've heard bad things about window companies" | BBB rating, reviews, warranty, local reputation |
| "Will new windows really save money?" | Energy savings calculator, ENERGY STAR data, customer testimonials |

### FAQ Questions to Include
1. How much do replacement windows cost?
2. How long does window installation take?
3. What's the best type of window for my home?
4. Will new windows save me money on energy bills?
5. What warranty do you offer?
6. Do you offer financing?
7. Can I customize my windows?
8. What happens to my old windows?

---

## Conversion Blueprint

### High-Converting Windows & Doors Landing Page Formula

```
┌─────────────────────────────────────────────────────────────┐
│  HERO SECTION                                                │
│  ├─ Headline: "40% Off Windows + Free Installation*"        │
│  ├─ Subheadline: Energy-efficient windows, expert install   │
│  ├─ Hero image: Beautiful home with new windows             │
│  ├─ CTA: "Get Your Free Quote"                              │
│  ├─ Phone: "Or Call (XXX) XXX-XXXX"                         │
│  └─ Trust strip: X+ Years | X Million Installed | Lifetime Warranty │
├─────────────────────────────────────────────────────────────┤
│  PRODUCT CATEGORIES (3-4)                                   │
│  ├─ Replacement Windows [image]                             │
│  ├─ Entry Doors [image]                                     │
│  ├─ Patio Doors [image]                                     │
│  └─ Each with "Explore Options" link                        │
├─────────────────────────────────────────────────────────────┤
│  ENERGY SAVINGS CALCULATOR                                  │
│  ├─ "See How Much You Could Save"                           │
│  ├─ Input: # of windows, home age, ZIP                      │
│  ├─ Output: Annual savings estimate                         │
│  └─ CTA: "Get Your Personalized Quote"                      │
├─────────────────────────────────────────────────────────────┤
│  WHY REPLACE YOUR WINDOWS                                   │
│  ├─ Lower Energy Bills (with % savings)                     │
│  ├─ Increased Home Value                                    │
│  ├─ Better Comfort Year-Round                               │
│  └─ Enhanced Curb Appeal                                    │
├─────────────────────────────────────────────────────────────┤
│  OUR PROCESS (4-5 steps)                                    │
│  ├─ Step 1: Free In-Home Consultation                       │
│  ├─ Step 2: Custom Measurement                              │
│  ├─ Step 3: Professional Installation                       │
│  ├─ Step 4: Final Walkthrough                               │
│  └─ Timeline: "Most installations complete in 1 day"        │
├─────────────────────────────────────────────────────────────┤
│  PRODUCT SHOWCASE                                           │
│  ├─ Window styles gallery                                   │
│  ├─ Material options (vinyl, wood, fiberglass)              │
│  ├─ Color/finish options                                    │
│  └─ ENERGY STAR badge                                       │
├─────────────────────────────────────────────────────────────┤
│  WHY CHOOSE US                                              │
│  ├─ Expert Installation Team                                │
│  ├─ Lifetime Warranty                                       │
│  ├─ ENERGY STAR Partner                                     │
│  └─ Thousands of 5-Star Reviews                             │
├─────────────────────────────────────────────────────────────┤
│  TESTIMONIALS                                               │
│  ├─ Customer quotes with before/after                       │
│  ├─ Energy savings testimonials                             │
│  ├─ Video testimonials                                      │
│  └─ Review platform widgets                                 │
├─────────────────────────────────────────────────────────────┤
│  FINANCING SECTION                                          │
│  ├─ "Affordable Monthly Payments"                           │
│  ├─ 0% APR for 12/24 months                                 │
│  ├─ Payment calculator                                      │
│  └─ "Easy qualification" messaging                          │
├─────────────────────────────────────────────────────────────┤
│  FAQ SECTION                                                │
│  ├─ Cost questions                                          │
│  ├─ Installation questions                                  │
│  ├─ Product questions                                       │
│  └─ Warranty questions                                      │
├─────────────────────────────────────────────────────────────┤
│  FINAL CTA SECTION                                          │
│  ├─ "Ready to Upgrade Your Windows?"                        │
│  ├─ Full quote form                                         │
│  ├─ Phone number                                            │
│  └─ Current promotion reminder                              │
└─────────────────────────────────────────────────────────────┘
```

### Key Metrics to Target
- **Conversion rate**: 5-10% for consultation requests
- **Appointment show rate**: Critical to track
- **Quote-to-close rate**: Monitor sales effectiveness
- **Average order value**: Track for lead quality

### A/B Testing Priorities
1. Promotion headlines (% off vs. $ off vs. free installation)
2. Energy savings calculator placement
3. Form length (short vs. detailed)
4. Product imagery (product shots vs. installed shots)
5. Urgency messaging (limited time vs. evergreen)

---

## Industry-Specific Insights

### What Makes Windows/Doors Different
- **High-ticket purchase** - Significant investment
- **Long sales cycle** - In-home consultations standard
- **Energy savings hook** - Major selling point
- **Financing important** - Monthly payment messaging
- **Promotional pricing** - Industry standard

### Pricing Psychology
- Show promotional pricing prominently
- "Starting at $X per window" messaging
- Monthly payment options
- Energy savings ROI calculations
- Compare to utility costs

### Seasonal Considerations
- Spring: Pre-summer cooling preparation
- Fall: Pre-winter heating preparation
- Winter: Energy bill pain point messaging
- Year-round: Comfort and curb appeal

### Lead Quality Factors
- Number of windows (higher = higher value)
- Home age (older = more urgency)
- Timeline (ASAP = hot lead)
- Financing needs (pre-qualification)

### Competition Positioning
- Renewal by Andersen: Premium, full-service
- Window World: Value-focused
- Pella/Marvin: Design-forward
- Differentiate based on target market
